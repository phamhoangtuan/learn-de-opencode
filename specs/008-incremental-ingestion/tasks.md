...
- [ ] T003 [US1] Add `ALTER TABLE ingestion_runs ADD COLUMN IF NOT EXISTS` migration guard for all four new columns (`files_checked INTEGER DEFAULT 0`, `files_skipped INTEGER DEFAULT 0`, `files_ingested INTEGER DEFAULT 0`, `files_failed INTEGER DEFAULT 0`) inside `create_tables()` in `src/ingestion/loader.py` (CHK005, FR-014)
...
- [ ] T012 [US2] Implement `ManifestStore.should_skip(self, file_path: str, current_hash: str) -> bool` — returns `True` only when a row exists with `status='success'` AND `file_hash=current_hash`; returns `False` for `failed`, `pending`, changed hash, or missing row (CHK028–CHK032, FR-003, FR-004, FR-005)
